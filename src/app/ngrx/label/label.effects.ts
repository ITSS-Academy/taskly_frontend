    ...
    return actions$.pipe(
      ...
      switchMap(({label}) => {
        return labelService.createLabel(label).pipe(
          map((label: any) =>
              listActions.updateLabelToCard({cardId: label.cardId, labels: label}),
            labelActions.createLabelSuccess({label: label}),
          ),
          catchError((error) => {
            return of(labelActions.createLabelFailure({error: error.message}));
          }),
        );
      }),
    );
    ...
    return actions$.pipe(
      ...
      switchMap(({id}) => {
        return labelService.getLabelsInBoard(id).pipe(
          ...
          catchError((error) => {
            return of(labelActions.getLabelsInBoardFailure({error: error.message}));
          }),
        );
      }),
    );
