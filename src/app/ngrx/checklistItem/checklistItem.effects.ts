...
export const createSubtask = createEffect(
  (actions$ = inject(Actions), checklistService = inject(ChecklistItemService)) => {
    return actions$.pipe(
      ...
      exhaustMap(({checklistItem}) => {
        return checklistService.createSubTask(checklistItem).pipe(
          mergeMap((result: any) => [
            listActions.addCountSubtask({subtask: result}),
            cardActions.addNewChecklistItem({checklistItem: result}),
            checklistItemActions.addNewChecklistItemSuccess(),
          ]),
          catchError((error) => {
            return of(
              checklistItemActions.addNewChecklistItemFailure({error: error.message}),
            );
          }),
        );
      }),
...
export const toggleSubtask = createEffect(
  (actions$ = inject(Actions), checklistService = inject(ChecklistItemService)) => {
    return actions$.pipe(
      ...
      exhaustMap(({checklistItem}) => {
        return checklistService.toogleSubTask({
          id: checklistItem.id!,
          ...
          catchError((error) => {
            return of(
              checklistItemActions.toggleChecklistItemFailure({error: error.message}),
            );
          }),
        );
      }),
    ...
    return actions$.pipe(
      ...
      exhaustMap(({checklistItemId}) => {
        return checklistService.deleteSubTask(checklistItemId).pipe(
          mergeMap(() => [
            // listActions.deleteCountSubtask({subtaskId: checklistItemId}),
            cardActions.deleteChecklistItem({checklistItemId}),
            checklistItemActions.deleteChecklistItemSuccess(),
          ]),
          catchError((error) => {
            return of(
              checklistItemActions.deleteChecklistItemFailure({error: error.message}),
            );
          }),
        );
      }),
